import numpy
from neuro_fuzzy_main import set_net_acrhitecture
...
def test_nn_arch():
	...
	for number in number_of_rules:
		...
		assert len(nn[0]) == number, "Number of input neurons is wrong!"
...
def test1(x, y, A, B):
	...
	"""
	p1, q1, r1 = 2, 3, 1
	f1 = fuzzy.calculate_f_for_rule(x, y, p1, q1, r1)
	p2, q2, r2 = 1, 1, 1
	f2 = fuzzy.calculate_f_for_rule(x, y, p2, q2, r2)
	initial_weights = numpy.random.random_sample(2, )
	rule_outputs = numpy.array([f1, f2])
	mean_weights = fuzzy.calculate_mean_weights(initial_weights, rule_outputs)
	output = fuzzy.calculate_output(mean_weights, rule_outputs)
	return output
...
if __name__ == '__main__':
	...
	A = fuzzy.membership_function(x, a=2, b=3)
	B = fuzzy.membership_function(y, a=1, b=1)
